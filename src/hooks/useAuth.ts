        ...
        const fetchSubData = async () => {
          ...
          try {
            ...
            if (p.role === 'student') {
              ...
              if (data) {
                // SECURITY: The store's `student.onboarded` must reflect ONLY the
                // students.onboarded column. Never derive it from the profile flag —
                // a single-flag derivation let drifted accounts (profiles.has_onboarded
                // true but no real onboarding) skip onboarding entirely. "Fully onboarded"
                // is defined in lib/onboarding.ts as BOTH flags being true.
                const derivedOnboarded = data.onboarded === true

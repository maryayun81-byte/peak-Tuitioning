      ...
      <div className="flex items-center gap-2">
        {STEP_LABELS.map((label, i) => (
          <div key={label} className="flex items-center gap-2">
            <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-black transition-all ${i <= step ? 'text-white' : 'text-[var(--text-muted)]'}`}
              style={{ background: i <= step ? 'var(--primary)' : 'var(--input)' }}>
              {i < step ? <Check size={14} /> : i + 1}
            </div>
            <span className={`text-xs font-bold hidden sm:block ${i === step ? 'text-[var(--text)]' : 'text-[var(--text-muted)]'}`}>{label}</span>
            {i < STEP_LABELS.length - 1 && <div className="w-6 h-px bg-[var(--card-border)]" />}
          </div>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {step === 0 && (
          <motion.div key="step0" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-5">
            ...
            <Card className="p-5 space-y-4">
              <h2 className="font-black text-base flex items-center gap-2" style={{ color: 'var(--text)' }}>
                <Clock size={18} className="text-blue-500" /> Timer Settings
              </h2>
              <Input
                label="Overall Session Duration (minutes, blank = untimed)"
              ...
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>
                Each question also gets its own per-question timer (set in Step 2).
              </p>
            </Card>
        ...
        {step === 1 && (
          <motion.div key="step1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex lg:flex-col gap-2 overflow-x-auto lg:overflow-x-visible pb-2 lg:pb-0 lg:w-40 shrink-0 custom-scrollbar">
                {questions.map((q, i) => (
                  <motion.button key={q.id}
                    initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                    onClick={() => setActiveQIdx(i)}
                    className={`shrink-0 w-24 lg:w-full text-left p-2.5 rounded-xl border text-xs font-bold transition-all ${activeQIdx === i ? 'text-white border-transparent' : 'border-[var(--card-border)] text-[var(--text-muted)]'}`}
                    ...
                    <div className="truncate font-normal opacity-80 mt-0.5 hidden lg:block">{q.text.slice(0, 20) || 'Untitled'}</div>
                  </motion.button>
                ))}
                <button onClick={addQuestion}
                  className="shrink-0 w-12 lg:w-full h-10 lg:h-auto p-2.5 rounded-xl border border-dashed text-xs font-bold flex items-center justify-center gap-1 transition-all hover:border-primary hover:text-primary"
                  style={{ borderColor: 'var(--card-border)', color: 'var(--text-muted)' }}>
                  <Plus size={14} /> <span className="hidden lg:inline">Add Q</span>
                </button>
              </div>
              ...
              {activeQ && (
                <motion.div key={activeQ.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex-1 min-w-0 space-y-4">
                  <Card className="p-5 space-y-4">
                    ...
                    <textarea
                      className="w-full rounded-xl p-3 text-sm resize-none border outline-none transition-all focus:ring-2 focus:ring-primary/30"
                      style={{ background: 'var(--input)', borderColor: 'var(--card-border)', color: 'var(--text)', minHeight: 80 }}
                    ...
                    <Input
                      ...
                      onChange={e => updateQ(activeQIdx, { image_url: e.target.value })}
                    />
                    ...
                    {activeQ.options.map((opt, oi) => (
                      <div key={opt.id} className="flex items-center gap-3">
                        <button
                          ...
                          className={`w-7 h-7 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${activeQ.correct_option_id === opt.id ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-[var(--card-border)]'}`}>
                          {activeQ.correct_option_id === opt.id && <Check size={14} />}
                        </button>
                        <input
                          className="flex-1 rounded-xl px-3 py-2 text-sm border outline-none transition-all focus:ring-2 focus:ring-primary/20"
                          style={{ background: 'var(--input)', borderColor: activeQ.correct_option_id === opt.id ? '#10B981' : 'var(--card-border)', color: 'var(--text)' }}
          ...
          <motion.div key="step2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-4">
            <Card className="p-5 space-y-3">
              ...
              <div className="flex flex-wrap gap-3 text-xs" style={{ color: 'var(--text-muted)' }}>
                ...
                {durationMinutes && <span className="flex items-center gap-1"><Clock size={12} /> {durationMinutes}min overall</span>}
